            ...
            - MPLS
                ...
                - LER
                    ...
                    - push: ラベルをつけてフォワーディング
                        - 20ビットのラベルを含む32ビットのラベル
                    - swap: ラベルを付け替えてフォワーディング
                    - pop: ラベルを外してフォワーディング
                    - LSP
                        - Label Switch Path
                        - 宛先や扱いが同じパケットは、どれもラベルによって決まる同一の道筋
                        - point2point LSP
                        - mergeLSP
                        - LSPを張るには、各LSRが隣接するLSRにMPLSラベル情報を配布するか、ルーティングプロトコルにラベル情報を載せてやりとりする
                - 利点
                    - 転送処理の高速化
                        - MPLSでは固定長のラベルを使用するため、処理が単純になり転送処理のハードウェア化による高速化が可能
                        - インターネットのバックボーンルーターでは莫大な量の経路制御表を記憶して処理する必要があるが、ラベルは必要な数だけ設定すればよく処理するデータ量が少なくて済む
                        - IPv4とIPv6の違いに関わらず、高速に転送処理可能
                    - ラベルを利用して仮想的なパスを張り、その上でIPなどのパケットを使った通信ができる
                        - MPLSを利用した通信品質の制御や帯域保証、VPNなどが提供
- アプリケーションプロトコル
    - 遠隔ログイン
        - TSS(Time Sharing System)のような環境を実現するアプリケーション
            - TSSでは中央に処理能力の高いコンピューターが存在し、そのコンピュータに、処理能力を持たない複数の端末が端末専用の通信回線で接続されていた
        - メインフレームと端末の関係をコンピュータネットワークに応用したもの
        - 汎用コンピューターやUNIXワークステーションなどのコンピュータにログインして、そのコンピュータのアプリケーションを利用したり、システムの環境設定をしたりする
        - TELNET
            - TCPのコネクションを1つ利用
            - 通信路を通して相手のコンピュータにコマンドが文字列として送信され、相手のコンピュータで実行される
            - 自分のキーボードとディスプレイが、相手のコンピュータの内部で動作しているシェルに接続しているイメージ
                - Shell
                    - OSを貝殻のように包み、OSが提供する機能をユーザーが利用しやすくしてくれるUIのこと。
                    - キーボードやマウスから入力されたユーザーのコマンドを解釈して、OSに実行させる
            - ルーターや高機能スイッチなどのネットワーク機器にログインして、その機器の設定を行うときにもよく使われる
            - ログインするためには、自分のログイン名とパスワードがその機器に登録されている必要がある
            - ログイン時のパスワードは平文
            - オプション
                - ユーザーが入力した文字以外にオプションをやりとりする機能
            - TELNETクライアント
                - TELNETプロトコルを利用して遠隔ログインをするときのクライアントプログラム
                - 通常接続先ホストのTCPポート23番
                - それ以外の番号のTCPポートに接続して、そのポートで待っているアプリケーションを実行することも可能
                    - `telnet HostName TCPPortNo`
                - TCP/IPのアプリケーションを開発するときのデバッグに利用されることもある
        - SSH
            - Secure Shell
            - 暗号化された遠隔ログインシステム
            - 通信内容が暗号化されるため、盗聴されたとしてもパスワードや入力したコマンド、コマンドの処理結果がわからなくなる
            - より強固な認証機能を利用するとことができる
            - ファイル転送ができる
            - ポートフォワード機能が利用できる
                - ポートフォワード
                    - 特定のポート番号に届けられたメッセージを、特定のIPアドレス、ポート番号に転送する仕組み
                    - SSHコネクションを経由する部分が暗号化されるため、セキュリティを確保した柔軟な通信が可能になる
                    - 通常ソフト - 通常のTCP通信 -> SSHクライアントプログラム - SSHが暗号化したTCP通信　-> SSHサーバープログラム - 通常のTCP通信 -> POPサーバー
            - 利用可能な認証方法
                - パスワード認証
                - 公開鍵認証
                    - 事前に公開鍵と秘密鍵を生成し、接続先に公開鍵を渡しておく
                    - パスワードをネットワーク上に流す必要がない
                - ワンタイムパスワード
    - FTP
        - File Transfer Protocol
        - 異なるコンピュータ間でファイルを転送する
        - 相手先のコンピュータにログインしてから操作する
        - anonymous ftp サーバー
            - 誰でもログインできるFTPサーバー
            - ログイン名: anonymous, ftp
        - 2つのTCPコネクションが利用される
            - 制御用
                - FTPの制御に利用
                - ログインのためのユーザー名とパスワードの確認や、転送するファイル名や転送方法の指示
                - ASCII文字列により要求と応答をやりとり
                - TCPポート番号21を利用
                - コマンドが実行される度に、データ転送用のTCPコネクションが確立される
                - ユーザーからの切断の指示があるまで接続されたまま。基本、FTPサーバーは一定時間ユーザーからコマンドが送られてこないと、コネクションを強制的に切断
            - データの転送用
                - 制御用のTCPコネクションとは逆向きに確立
                - NATを介して外部のFTPサーバーを利用する場合などには、そのままデータ転送用のTCPコネクションを確立することはできない。そのためPASVコマンドを投入して、データ転送用TCPコネクションの向きを変更する
                - TCPポート番号20を利用
                - PORTコマンドを利用して別のポート番号を指定することも可能
                - セキュリティ向上のため、データ転送用のコネクションのポート番号を乱数的に割り当てるのが普通
    - 電子メール
        - SMPT
            - Simple Mail Transfer Protocol
            - TCPの利用
            - 電源を切らないメールサーバーを経由
            - TCPポート番号25を利用
            - 1つのTCPコネクションを確立して、そのコネクション上で制御や応答、データからなるメッセージの転送を行う
            - クライアントはテキストコマンドで要求を出し、サーバーは3桁の数字で表される文字列で応答を返す
        - POP
            - Post Office Protocol
            - 電子メールを受信するためのプロトコル
            - メールは送信者からSMTPによって常時電源が入っているPOPサーバーまで到着する
            - クライアントはPOPによってPOPサーバーに保存された相手からのメールを取り出す。
            - また他人にメールを盗み取られることを防ぐためにユーザーの認証を行う
        - IMAP
            - Internet Message Access Protocol
            - 電子メールなどのメッセージを受信するためのプロトコル
            - 電子メールをサーバー側で管理
            - サーバー上の電子メールの全てをダウンロードしなくても電子メールを読める
            - サーバー側でMIMEの情報を処理
            - 未読/既読情報の処理、メールを仕分けするときのメールボックスの管理
        - メールアドレス
            - 電子メールの配送先の管理はDNSによって行われる
            - MXレコード
                - DNSにはメールアドレス(@以下)、そのメールアドレス宛のメールを送信すべきメールサーバーのドメイン名を登録
                - MXレコードを用いることでメールサーバーを正確に指定することで、異なるメールアドレスを特定のメールサーバーで管理
        - MIME
            - Multipourpose Internet Mail Extensions
            - インターネットで幅広く使えるようにメールのデータ形式を拡張したもの
            - ヘッダと本文
            - ヘッダと本文を空行で区切る
            - MIMEヘッダの「Content-Type」で「Multiple/Mixed」を指定し、「boundary = 」の後に書いた文字列で仕切ると、1つのMIMEメッセージを複数のMIMEメッセージの集合として定義する
        - 迷惑メールへのさまざまな対策
            - メールの送信元ユーザーを認証する仕組み
                - POP before SMTP
                    - メール送信の前にPOPによるユーザー認証を行い、人書グア正しければ、一定期間クライアントIPアドレスからのSMTP通信を受け入れる仕組み
                - SMTP認証
                    - メール送信時にSMTPサーバーで、ユーザー認証を行うようにしたSMTPの拡張仕様
            - 送信元ドメインを認証する仕組み
                - SPF
                    - Sender Policy Framework
                    - 送信元メールサーバーのIPアドレスをDNSサーバーに登録しておき、受信側で受信したメールのIPアドレスと送信元メールサーバーのIPアドレスを比較しドメイン認証することで、受信したメールの送信音が詐称されているかどうかを確認する
                - DKIM
                    - DomainKeys Identified Mail
                    - 送信元メールサーバーで電子署名を付与し、受信側では、電子署名を認証することで、受信したメールの送信元が詐称されているかどうかを確認する仕組み
                    - 送信元メールサーバーで署名に使用する公開鍵をDNSサーバーに登録しておき、受信側が公開鍵を取得し署名を認証できるようにする
                - DMARC
                    - Domain-based Message Authentication, Reporting and Conformance
                    - SPFやDKIMなどの送信元ドメインを認証する仕組みにおいて、認証が失敗したときのメールの取り扱いのポリシーを送信者がDNSサーバーに登録して公開する仕組み。
                    - 受信側は、認証に失敗した場合に送信者のポリシーによって取り扱いを決定したり、送信者へ認証に失敗したことを通知したりすることができる
            - 他
                - OP25B
                    - Outbout Port 25 Blocking
                    - インターネットサービスプロバイダなどで、迷惑メールやウイルスメールを直接送信できないように、TCPポート25のSMTP通信をブロックする対策
    - WWW
        - World Wide Web
        - インターネット上の情報をハイパーテキスト形式で参照できる情報提供システム
        - 文書中に別の文書へのリンクを記述しておくことで、インターネット上の文書を相互に参照可能
        - Webブラウザ
            - WWWの情報を画面に表示するクライアントソフトウェア
        - URI
        ...
        - XML
            ...
            - データをファイルに保存したり、アプリケーション間でやりとりしたりする形式
        - HTML5/CSS3
        - HTTP
            - HyperText Transfer Protocol
            - HTML文書や画像、音声、動画などのコンテンツの送受信に用いられるプロトコル
            - クライアントがHTTPサーバーに情報を要求し、この要求に対して、HTTPサーバーがクライアントに情報を送信する
            - ユーザーがブラウザにWebページのURIを入力するとHTTPの処理が開始される。通常TCPポート80が利用される。クライアントからサーバーへポート80番でTCPのコネクションの確立。
            - HTTP1.0
                - 1つのコマンドや応答をやりとりするたびに、TCPコネクションを確立し切断
            - HTTP1.1
                - 1つのコネクションで複数のコマンドと応答のやりとりが可能になった
            - 認証方式
                - HTTPサーバーではクライアントを認証し、認証に成功したクライアントにのみコンテンツを返すようにアクセス制限できる
                - Basic認証
                    - base64でエンコードされるが、ユーザーIDとパスワードが平文でネットワークを流れるので安全ではないため、HTTPSの暗号化通信と組み合わせる
                - Digest認証
                    - ユーザーIDとパスワードをMD5でハッシュ化して送信する
                    - 解析可能になりつつある
            - 処理の流れ
                1. クライアント
                    1. Webブラウザに検索キーワードを入力する
                    1. JavaScriptによる処理が行われる
                    1. HTTPによるリクエストメッセージがWebサーバーに送られる
                1. Webサーバー
                    1. HTTPリクエストを解析し、必要に応じてCGIにより外部プログラムを呼び出す
                1. Webアプリケーションサーバー
                    1. プログラム処理を行う
                1. DBサーバー
                    1. DBで検索処理を行う
                    1. DBの処理結果をプログラムに応答する
                1. Webアプリケーションサーバー
                    1. 処理結果に基づいてHTML文書を作成する
                1. Web サーバー
                    1. HTTPレスポンスをクライアントに送信する
                1. Webブラウザ
                    1. HTMLを受信し、ブラウザにデータを渡す
                    1. JSによるブラウザ側の処理を行う
                    1. ブラウザの画面に処理結果を表示
            - Cookie
                - ユーザー情報を識別するため
                - Webサーバーがクライアント側に情報（タグと値）を格納できる
                - ログイン情報やネットショッピングでの買い物かごの情報など
            - WebSocket
                - クライアントとサーバー間の双方向通信をHTTP上で実現するプロトコル
                - まずクライアントとサーバー間でHTTP通信を行い、HTTPのupgradeリクエスト/レスポンスでWebSocket用の通信路を確立し、双方向の通信が行えるようにする
    - SNMP
        - Simple Network Management Protocol
        - ネットワークに接続されている機器に対して、遠隔地から設定を更新したり、正常に動作しているかをチェックできる
        - マネージャ
            - ネットワーク監視端末
        - エージェント
            - ルーターやスイッチなど
        - マネージャとエージェント間の通信のやり取りを定めたもの
        - MIBと呼ばれるエージェントが管理している管理情報のデータベースの値を見ることと、新しい値をセットできる
            - MIB
                - Management Information Base
                - ツリー型のデータベース
                - SNMPがMIBにアクセスするときには数字の列が使われる
                - SNMPのプレゼンテーション層
                - 衝突の回数やトラフィックの量などの情報収集、インタフェースのIPアドレスなどの情報の変更、ルーティングプロトコルの停止・起動、機器の再起動や電源OFFなどの処理を行うことができる
            - RMON
                - Remote Monitoring MIB
                - 接続されるネットワークの回線を監視するパラメータ群
                - トラフィックの統計情報などユーザー側の立場から考えて有意義な情報
        - メッセージ処理、セキュリティ、アクセスコントロール部分に分けて、それぞれに必要な仕組みを選択できるようにする
- セキュリティ
    - サイバーセキュリティ
        - 関係のない第三者への情報漏洩を防止し、情報システムおよび通信ネットワークの安全性、信頼性を確保するため、必要な措置が取られ、適切に維持管理されていること
    - SOC
        - Security Operation Center
        - ネットワークやパソコンなどの端末を監視し、企業に設けられたサイバー攻撃を検出して分析や対策を検討する部門や専門組織のこと
        - インシデントの検知に重点を置く
    - CSRIT
        - Computer Security Incident Response Team
        - ネットワークやパソコンなどで何らかのセキュリティ上の問題が万が一発生した場合に、それらのインシデントに対応するチームのこと
        - インシデント発生後の対応に重点を置く
    - セキュリティの構成要素
        - ファイアウォール
            - 組織内のネットワークとインターネットを接続するときには、組織内部のネットワークを不正アクセスから守るためにファイアウォールを設置する必要がある
            - パケットフィルタリングタイプ、アプリケーションゲートウェイタイプ
        - IDS
            - Intrusion Detection System
            - 一旦内部などに侵入して、不正アクセスを行う通信を見つけ、セキュリティの管理者などに通知をする
        - IPS
            - Intrusion Prevention System
            - IDSの持つネットワークの監視、異常検知機能、不正侵入を防御する機能
        - DMZ
            - Demilitarized Zone
            - 外部からも内部からも隔離された専用のサブネット
        - WAF
            - Web Application Firewall
            - ウェブアプリケーションの脆弱性を悪用する攻撃から守るためのセキュリティ対策
            - ウェブアプリケーションが稼働するWebサーバーの全面に配置することで、ファイアウォールやIDS/IPSでは検知することが難しい「SQLインジェクション」や「クロスサイトスクリプティング」「パラメータ改竄」などのアプリケーションレベルでの攻撃を検知し、防ぐことが可能
    - 暗号化
        - ある値を用意し、その値を使って元のデータに対して一定のアルゴリズムによる変換を行い、暗号化データを作る
        - 共通鍵暗号方式
            - 暗号化と複合に同じ鍵を使う
            - 安全な鍵の受け渡し方法が課題
        - 公開鍵暗号方式
            - 暗号化と複合に一対の別々の鍵を使う
            - 秘密鍵を厳重に管理し、公開鍵はメールで送付したりWebで公開視したりPKIを利用して配布する
                - PKI
                    - Public Key Infrastructure
                    - 通信の相手が本物かどうかを信頼できる第三者に証明してもらうための仕組み
                    - CAが発行する証明書により、通信内てが本物かどうかを確かめる
    - セキュリティのためのプロトコル
        - VPN
            - Virtual Private Network
        ...
        - IPsec
            - VPNを構築するときに利用
            - IPヘッダの後ろに暗号ヘッダや認証ヘッダをつける
            - ヘッダ以降のデータを暗号化して、解読できないようにする
            - 途中でデータが改竄されたときには、改竄されたことを判別できるようになる
            - ネットワーク層のプロトコル
            - 構成要素
                - ESP
                ...
                - IKE
                    ...
                    - UDPポート50番
            - SA
                - Secirotu Asspcoatopm
                - IPSecを使ってお互いに通信する機器はpeerの関係になる
                - セキュリティプロトコル、通信モード、暗号化方式などIPsec通信に必要なパラメータの集まり
                - 双方向のIPsec通信を行う場合は、2つのSAを確立
            - 手順
                - IKEにより共通鍵を生成するためのパラメータを相互に交換し、お互いで共通鍵を生成してISAKMP SAを確立し、ピアの認証を行う
                - その後IPsec SAを確立するためのパラメータをISAKMP SAによる暗号化通信でやりとりし、IPSec SAを確立した後、IPSec SAのセキュリティプロトコル、通信モード、暗号化方式などに従って暗号化通信を行う
                - 受信後は複合して認証の検証を行い、上位レイヤにパケットを渡す
    - TLS/SSL
        - Transport Layer Security/Secure Sockets Layer
        - HTTPS
            - TLS/SSLを使ったHTTP通信
            - 共通鍵暗号方式で暗号化処理
            - 共通鍵を送信するときに公開鍵暗号方式を使う
            - 手順
                - WebサーバーにHTTPSでアクセス
                - Webサーバーの公開鍵と証明書を送信
                    - 使用するアルゴリズムを通知
                - 電子証明書を確認して、公開鍵がアクセスしたサーバーのものかどうかを確認
                    - 電子証明書を認証局の公開鍵で復号する
                - 共通鍵暗号化方式の共通鍵を作成する。Webサーバーの公開鍵を使って暗号化して送信
                - 暗号を復号して、共通鍵を知る
                    - Webサーバーの秘密鍵で復号する
                - 共通鍵方式による暗号化送信が可能
        - SSL-VPN
            - TLS/SSLを利用したリモートアクセスVPN
            - 外部の端末からインターネット経由で組織のVPN装置に接続し、暗号化通信によってVPN接続を可能にする
            - セッション層での暗号化通信
    - IEEE802.1X
        ...
        - 構成要素
            - クライアントの端末
            - アクセスポイントとしての無線基地局やレイヤ2スイッチ
            - 認証サーバー
                - RADIUSサーバー
        - 手順
            - 未確認の端末からの接続要求
            - 認証サーバーのあるVLANへの一時的なIPアドレスを付与
                - 端末は認証サーバーとしか接続できない極めて限られたネットワークに接続
            - 利用者はユーザーIDやパスワードの入力を要求される
            - 認証サーバーが確認され、利用者が利用可能なネットワーク情報がアクセスポイントと端末に通知される
            - アクセスポイントが、その端末がネットワークに接続するために必要となるVLAN番号へ、接続の切り替えを行う
            - 端末側ではVLANの切り替え後にIPアドレスをリセットして再設定を行うことでネットワークの利用が可能になる